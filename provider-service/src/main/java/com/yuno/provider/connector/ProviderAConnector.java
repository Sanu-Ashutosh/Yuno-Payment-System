...
public class ProviderAConnector implements PaymentProvider {
    ...
    @Value("${providers.provider-a.success-rate:0.85}")
    private double successRate;

    @Value("${providers.provider-a.min-latency-ms:100}")
    private int minLatencyMs;

    @Value("${providers.provider-a.max-latency-ms:300}")
    ...
    public ProviderProcessResponse process(ProviderProcessRequest request) {
        log.info("ProviderA processing payment. paymentId={}, amount={} {}",
        ...
        if (random.nextDouble() < successRate) {
            ...
            String txId = "PA-" + UUID.randomUUID().toString().substring(0, 8).toUpperCase();
            log.info("ProviderA SUCCESS. paymentId={}, txId={}", request.getPaymentId(), txId);
            return ProviderProcessResponse.success(txId, ProviderType.PROVIDER_A, false, 0, 0L);
        } else {
            String reason = pickFailureReason();
            log.warn("ProviderA FAILED. paymentId={}, reason={}", request.getPaymentId(), reason);
            throw new ProviderException("PROVIDER_A", reason, isRetryable(reason));
        }
    }
    ...
    private String pickFailureReason() {
        String[] reasons = {
            "INSUFFICIENT_FUNDS", "CARD_DECLINED", "CARD_EXPIRED",
    ...
    private boolean isRetryable(String reason) {
        return reason.equals("PROVIDER_TIMEOUT") || reason.equals("NETWORK_ERROR");
    }
}
